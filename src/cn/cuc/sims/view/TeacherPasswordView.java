...
public class TeacherPasswordView extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TeacherPasswordView frame = new TeacherPasswordView("");
	 ...
	 * Create the frame.
	 */
	public TeacherPasswordView(String id) {
		user = new UserAction();
		this.id = id;
	...
	private void button_1ActionPerformed(java.awt.event.ActionEvent evt) {
		TeacherView st = new TeacherView(id);
		st.setVisible(true);
		this.dispose();
	}
